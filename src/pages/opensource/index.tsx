...
import { openSourceData } from '@site/data/opensource'
import styles from './opensource.module.css'

function CategorySidebar() {
  const sidebar = {
    title: '',
    items: openSourceData.map(w => ({ title: w.name, permalink: `#${w.name}` })),
  }
        ...
        <div key={cate.name}>
          ...
          <section>
            <ul className={styles.opensourceList}>
              {cate.opensources.map(opensource => (
                <OpenSourceCard key={opensource.name} opensource={opensource} />
              ))}
            </ul>
          </section>
        </div>
